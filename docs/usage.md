...
Agent-core 同时支持两种 skill 体系：

### 1. SKILL.md 生态（推荐，兼容 opencode/claude-code/hermes）
...
### 2. Python ABC Skill（旧体系，用于代码级拦截）
...
class MyCustomSkill(Skill):
    name = "custom_action"
    description = "处理自定义 action"

    def can_handle(self, parsed, messages):
        return parsed.get("action", {}).get("method") == "CUSTOM"

    def execute(self, parsed, messages, context):
        yield ("status", {"message": "执行自定义操作..."})
        yield ("step_done", {"step": 1, "ok": True, "path": "/custom"})
        yield ("__continue__", {})
