...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="nanoghost")
    sub = parser.add_subparsers(dest="cmd")

    inst = sub.add_parser("instance")
    inst_sub = inst.add_subparsers(dest="inst_cmd")
    inst_list = inst_sub.add_parser("list")
    inst_list.set_defaults(func=_cmd_instance_list)
    inst_path = inst_sub.add_parser("path")
    inst_path.add_argument("name")
    ...
    gw_start.add_argument("--instance-dir", "-I", default=None)
    gw_start.add_argument("--host", default="127.0.0.1")
    gw_start.add_argument("--port", type=int, default=0)
    gw_start.set_defaults(func=_cmd_gateway_start)

    gw_status = gw_sub.add_parser("status")
    gw_status.add_argument("--instance-dir", "-I", default=None)
    gw_status.add_argument("--host", default=None)
    gw_status.add_argument("--port", type=int, default=0)
    gw_status.set_defaults(func=_cmd_gateway_status)

    gw_stop = gw_sub.add_parser("stop")
    gw_stop.add_argument("--instance-dir", "-I", default=None)
    gw_stop.add_argument("--host", default=None)
    gw_stop.add_argument("--port", type=int, default=0)
    gw_stop.set_defaults(func=_cmd_gateway_stop)

    gw_restart = gw_sub.add_parser("restart")
    gw_restart.add_argument("--instance-dir", "-I", default=None)
    gw_restart.add_argument("--host", default=None)
    gw_restart.add_argument("--port", type=int, default=0)
    gw_restart.set_defaults(func=_cmd_gateway_restart)

    gw_health = gw_sub.add_parser("health")
    gw_health.add_argument("--instance-dir", "-I", default=None)
    gw_health.add_argument("--host", default=None)
    gw_health.add_argument("--port", type=int, default=0)
    gw_health.set_defaults(func=_cmd_gateway_health)

    mcp = sub.add_parser("mcp")
    mcp_sub = mcp.add_subparsers(dest="mcp_cmd")

    p_list = mcp_sub.add_parser("list")
    p_list.set_defaults(func=_cmd_mcp_list)

    p_probe = mcp_sub.add_parser("probe")
    p_probe.add_argument("--server", default=None)
    p_probe.add_argument("--instance-dir", "-I", default=None)
    p_probe.set_defaults(func=_cmd_mcp_probe)

    p_tools = mcp_sub.add_parser("tools")
    p_tools.add_argument("server_id")
    p_tools.add_argument("--instance-dir", "-I", default=None)
    p_tools.set_defaults(func=_cmd_mcp_tools)

    p_reload = mcp_sub.add_parser("reload")
    p_reload.add_argument("--instance-dir", "-I", default=None)
